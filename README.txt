...
* You can define the **capacity** for each time-slot. 
* A **notification** email is sent to the specified email addresses (one or more) after completed the booking payment.
* A **confirmation** email is sent to the user after completing the booking payment.
* You can **assign a user** to the calendar. Users with "Editor Access Level" will get access to the calendar only if it has been assigned previously.
* Exports to **iCal** format (Google Calendar, Outlook).
* Includes **captcha** validation for preventing spam.
* The calendar has a **printable list** of bookings.
